...
APPIMAGE="$({ find "$APPIMAGE_DIR" -maxdepth 1 -type f -name 'WaveLinux_*_amd64.AppImage' -print 2>/dev/null || true; } | sort -V | tail -n1)"
...
INSTALLED_APPIMAGE="$SUPPORT_DIR/$(basename "${APPIMAGE:-WaveLinux.AppImage}")"
...
if [[ -z "$APPIMAGE" || ! -f "$APPIMAGE" ]]; then
  echo "Missing WaveLinux AppImage in $APPIMAGE_DIR" >&2
...
rm -f "$SUPPORT_DIR"/WaveLinux_*_amd64.AppImage
