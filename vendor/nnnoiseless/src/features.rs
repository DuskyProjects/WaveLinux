...
impl DenoiseFeatures {
    ...
    /// Apply neural denoising gains and synthesize one output frame.
    ///
    /// `previous_gain` carries the same inter-frame smoothing state used by
    /// [`DenoiseState`](crate::DenoiseState). This narrow split-stage hook lets
    /// callers run the neural network in an isolated process while retaining
    /// nnnoiseless' native feature extraction and overlap-add synthesis.
    pub fn apply_denoise_gains_and_synthesize(
        &mut self,
        out: &mut [f32],
        gain: &[f32; NB_BANDS],
        previous_gain: &mut [f32; NB_BANDS],
    ) {
        let mut smoothed = *gain;
        self.pitch_filter(&smoothed);
        for (gain, previous) in smoothed.iter_mut().zip(previous_gain.iter_mut()) {
            *gain = gain.max(0.6 * *previous);
            *previous = *gain;
        }
        let mut interpolated = [1.0; FREQ_SIZE];
        crate::interp_band_gain(&mut interpolated, &smoothed);
        self.apply_gain(&interpolated);
        self.frame_synthesis(out);
    }

    /// Synthesize one frame without applying neural gains.
    pub fn synthesize_unmodified(&mut self, out: &mut [f32]) {
        self.frame_synthesis(out);
    }
