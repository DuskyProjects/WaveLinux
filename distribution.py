...
class InstallState:
    ...
    wrapper_mode: str
    wrapper_source_dir: str | None
    wrapper_bundle_exec: str | None
...
@dataclass(frozen=True)
class SourceLauncherInstallResult:
    source_dir: str
    desktop_path: str
    icon_path: str
    wrapper_path: str
...
@dataclass(frozen=True)
class SourceLauncherRepairResult:
    source_dir: str
    wrapper_path: str
    desktop_path: str
    removed_entries: tuple[str, ...]
...
@dataclass(frozen=True)
class BundleLauncherInstallResult:
    bundle_path: str
    desktop_path: str
    icon_path: str
    wrapper_path: str
...
@dataclass(frozen=True)
class BundleLauncherRepairResult:
    bundle_path: str
    wrapper_path: str
    desktop_path: str
    removed_entries: tuple[str, ...]
...
def _wrapper_source_dir(path: str) -> str | None:
    for raw_line in _read_text(path).splitlines():
        line = raw_line.strip()
        if not line.startswith("SOURCE_DIR="):
            continue
...
def _wrapper_bundle_exec(path: str) -> str | None:
    for raw_line in _read_text(path).splitlines():
        line = raw_line.strip()
        if not line.startswith("BUNDLE_EXEC="):
            continue
        value = line.split("=", 1)[1].strip().strip('"').strip("'")
        if not value:
            return None
...
def _write_source_launchers(source_dir: str, *, home_dir: str) -> tuple[str, str]:
    ...
    wrapper_contents = (
        "#!/bin/sh\n"
        "# Auto-generated by WaveLinux source install.\n"
...
def _write_bundle_launchers(bundle_path: str, *, home_dir: str) -> tuple[str, str]:
    ...
    wrapper_contents = (
        "#!/bin/sh\n"
        "# Auto-generated by WaveLinux local bundle install.\n"
        f'BUNDLE_EXEC="{bundle_path}"\n'
        'if [ ! -x "$BUNDLE_EXEC" ]; then\n'
        '    echo "WaveLinux bundled binary not found at $BUNDLE_EXEC." >&2\n'
        '    echo "Repair launchers from the current WaveLinux build, or install a verified AppImage." >&2\n'
        "    exit 1\n"
        "fi\n"
        'exec "$BUNDLE_EXEC" "$@"\n'
    )
...
def install_state(*, home=None, environ=None, argv=None) -> InstallState:
    ...
    wrapper_source_dir = _wrapper_source_dir(wrapper) if os.path.exists(wrapper) else None
    wrapper_bundle_exec = _wrapper_bundle_exec(wrapper) if os.path.exists(wrapper) else None
    wrapper_mode = "missing"
    if os.path.exists(wrapper):
        if wrapper_target == os.path.abspath(installed_appimage):
            wrapper_mode = "appimage"
        elif wrapper_source_dir:
            wrapper_mode = "source"
        elif wrapper_bundle_exec:
            wrapper_mode = "bundle"
        else:
    ...
    if wrapper_mode == "source" and (
        not wrapper_source_dir
        or not os.path.exists(os.path.join(wrapper_source_dir, "main.py"))
    ):
        warnings.append("Installed source wrapper points at a missing WaveLinux checkout.")
    if wrapper_mode == "bundle" and (
        not wrapper_bundle_exec
        or not os.path.isfile(wrapper_bundle_exec)
        or not os.access(wrapper_bundle_exec, os.X_OK)
    ):
        warnings.append("Installed bundle launcher points at a missing WaveLinux binary.")
    if desktop_mismatch:
    ...
    return InstallState(
        ...
        wrapper_bundle_exec=wrapper_bundle_exec,
...
def install_source_checkout(source_dir: str, *, home=None) -> SourceLauncherInstallResult:
    source_dir = os.path.abspath(str(source_dir or ""))
    main_py = os.path.join(source_dir, "main.py")
    if not os.path.isfile(main_py):
        raise RuntimeError(f"WaveLinux source checkout not found at {source_dir}.")

    home_dir = os.path.expanduser("~") if home is None else os.path.abspath(home)
    target_icon = installed_icon_path(home=home_dir)
    os.makedirs(os.path.dirname(target_icon), exist_ok=True)
    icon_source = os.path.join(source_dir, "icon.png")
    if os.path.exists(icon_source):
        shutil.copy2(icon_source, target_icon)
    target_wrapper, target_desktop = _write_source_launchers(
        source_dir,
        home_dir=home_dir,
    )
    _refresh_desktop_database(os.path.dirname(target_desktop))
    return SourceLauncherInstallResult(
...
def install_current_source_checkout(*, home=None) -> SourceLauncherInstallResult:
    source_dir = os.path.dirname(resource_path("main.py"))
    return install_source_checkout(source_dir, home=home)


def install_bundle_binary(bundle_path: str, *, home=None) -> BundleLauncherInstallResult:
    bundle_path = os.path.abspath(str(bundle_path or ""))
    if not os.path.isfile(bundle_path) or not os.access(bundle_path, os.X_OK):
        raise RuntimeError(f"WaveLinux bundle binary not found at {bundle_path}.")
    ...
    if os.path.exists(icon_source):
        shutil.copy2(icon_source, target_icon)
    target_wrapper, target_desktop = _write_bundle_launchers(
        bundle_path,
        home_dir=home_dir,
    )
    _refresh_desktop_database(os.path.dirname(target_desktop))
    return BundleLauncherInstallResult(
...
def install_current_bundle(*, home=None) -> BundleLauncherInstallResult:
    mode = runtime_mode(home=home)
    if mode.kind != "bundle":
        raise RuntimeError("WaveLinux is not running from a local bundled binary.")
    return install_bundle_binary(mode.running_path, home=home)


def repair_source_checkout_launchers(source_dir: str, *, home=None) -> SourceLauncherRepairResult:
    source_dir = os.path.abspath(str(source_dir or ""))
    if not os.path.isfile(os.path.join(source_dir, "main.py")):
        raise RuntimeError(f"WaveLinux source checkout not found at {source_dir}.")
    ...
    for entry in state.stale_launcher_entries:
        try:
            ...
            continue
    result = install_source_checkout(source_dir, home=home_dir)
    _refresh_desktop_database(os.path.dirname(result.desktop_path))
    return SourceLauncherRepairResult(
...
def repair_current_source_checkout_launchers(*, home=None) -> SourceLauncherRepairResult:
    source_dir = os.path.dirname(resource_path("main.py"))
    return repair_source_checkout_launchers(source_dir, home=home)


def repair_bundle_launchers(bundle_path: str, *, home=None) -> BundleLauncherRepairResult:
    bundle_path = os.path.abspath(str(bundle_path or ""))
    if not os.path.isfile(bundle_path) or not os.access(bundle_path, os.X_OK):
        raise RuntimeError(f"WaveLinux bundle binary not found at {bundle_path}.")
    ...
    for entry in state.stale_launcher_entries:
        try:
            ...
            continue
    result = install_bundle_binary(bundle_path, home=home_dir)
    _refresh_desktop_database(os.path.dirname(result.desktop_path))
    return BundleLauncherRepairResult(
...
def repair_current_bundle_launchers(*, home=None) -> BundleLauncherRepairResult:
    mode = runtime_mode(home=home)
    if mode.kind != "bundle":
        raise RuntimeError("WaveLinux is not running from a local bundled binary.")
    return repair_bundle_launchers(mode.running_path, home=home)
